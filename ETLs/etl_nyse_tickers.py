...
df = get_tickers.GetTickerClient().pullList('nyse')
...
sqll_util.execute_statement(con, "delete from def_tickers where exchange = 'NYSE';")


data_load.DataLoadClient().loadInsertData(df, "def_tickers", con )
    #close(filename)
print(sqll_util.execute_query(con,"select * from def_tickers where exchange = 'NYSE';"))
